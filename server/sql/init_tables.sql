...
CREATE TABLE `correct_answer` (
  ...
  CONSTRAINT `playerOne`
    FOREIGN KEY (`player1`)
    REFERENCES `users` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `playerTwo`
    FOREIGN KEY (`player2`)
    REFERENCES `users` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `gameTopic`
    FOREIGN KEY (`topicId`)
    REFERENCES `topic` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION);
...
CREATE TABLE `user_answers` (
  ...
  CONSTRAINT `game_questions`
    FOREIGN KEY (`game_question_id`)
    REFERENCES `game_questions` (`gameId`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `user_playing`
    FOREIGN KEY (`userId`)
    REFERENCES `users` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
...
CREATE TABLE `game_points` (
  ...
  CONSTRAINT `user_game_points`
    FOREIGN KEY (`userId`)
    REFERENCES `users` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `points_user_in_game`
    FOREIGN KEY (`gameId`)
    REFERENCES `game` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE);
